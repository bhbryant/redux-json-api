...
const apiHydrate = createAction('API_HYDRATE');

describe('[State mutation] Hydration of links', () => {
  it('should not hydrate links into state when indexLinks is blank', () => {
    const state = {};

    const indexPayload = postsPayload;
    indexPayload.options = { };

    const updatedState = reducer(state, apiHydrate(postsPayload));

    expect(updatedState.posts.data.length).toEqual(postsPayload.data.length);
    expect(updatedState.links).toEqual(undefined);
  });

  it('should hydrate links into state when indexLinks string is present', () => {
    const state = {};
